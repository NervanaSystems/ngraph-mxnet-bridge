...
void Emitter::CreateUnaryOps() {
  ...
  ngraph_op_funcs_["Dropout"] = [this](const NodePtr node) {
    ...
    bool is_training =
        (mode == "always") || (exe_mode_ == GraphExeMode::kTrain);

    NgraphNodePtr out, gm;
    ...
    if (!is_training || p == 1) {
      out = input;
      gm = makeConstant(dtype, shape, 1.0f);
    } else {
      auto training = makeConstant(dtype, ngraph::Shape{}, 1);
      gm = std::make_shared<ngraph::op::GenerateMask>(training, shape, dtype,
                                                      seed, p);
      auto ng_p = makeConstant(dtype, shape, 1.0f / p);
      out = input * gm * ng_p;
    }

    multi_output_map_[node] = {out,
                               std::make_shared<ngraph::op::StopGradient>(gm)};
    return out;
  };
  ...
  ngraph_op_funcs_["Crop"] = [this](const NodePtr& node) {
    ...
    // use second input shape as crop reference
      ...
      throw std::runtime_error(
          "Crop unexpected number of inputs (expecting 1 or 2): " +
